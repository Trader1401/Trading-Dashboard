...
export default function Analytics() {
  ...
  // Only calculate analytics for active strategy trades
  const totalPnL = calculateTotalPnL(filteredTrades, strategies);
  ...
  const activeFilteredTrades = getActiveStrategyTrades(filteredTrades, strategies);
  const winningTrades = activeFilteredTrades.filter(t => parseFloat(t.profitLoss?.toString() || "0") > 0);
  const losingTrades = activeFilteredTrades.filter(t => parseFloat(t.profitLoss?.toString() || "0") < 0);
  const bestTrade = activeFilteredTrades.reduce((best, trade) => {
  ...
  }, activeFilteredTrades[0]);
  const worstTrade = activeFilteredTrades.reduce((worst, trade) => {
    const pnl = parseFloat(trade.profitLoss?.toString() || "0");
  ...
  // Risk metrics
  const avgTradeSize = activeFilteredTrades.length > 0 
    ? activeFilteredTrades.reduce((sum, trade) => {
        const entryPrice = parseFloat(trade.entryPrice?.toString() || "0");
        return sum + (entryPrice * trade.quantity);
      }, 0) / activeFilteredTrades.length
    : 0;

  const sharpeRatio = calculateSharpeRatio(activeFilteredTrades);
  const maxConsecutiveLosses = calculateMaxConsecutiveLosses(activeFilteredTrades);
  const maxConsecutiveWins = calculateMaxConsecutiveWins(activeFilteredTrades);
  const metrics = [
    {
      title: "Total P&L",
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white-900">Analytics</h1>
      ...
      <Tabs defaultValue="overview" className="mb-8">
        ...
        <TabsContent value="overview" className="space-y-8">
          ...
          >
            <PnLChart trades={filteredTrades} strategies={strategies} />
            <WinRateChart trades={filteredTrades} strategies={strategies} />
          </motion.div>
          ...
          >
            <EquityCurve trades={filteredTrades} strategies={strategies} />
          </motion.div>
        </TabsContent>
        ...
        <TabsContent value="performance" className="space-y-8">
          <motion.div 
            ...
            transition={{ duration: 0.5 }}
          >
            <MonthlyPerformanceChart trades={filteredTrades} strategies={strategies} />
            <Card>
              <CardHeader>
        ...
        <TabsContent value="strategies" className="space-y-8">
          ...
          >
            ...
            <TradeFrequencyChart trades={filteredTrades} strategies={strategies} />
            <Card>
              <CardHeader>
        ...
        <TabsContent value="psychology" className="space-y-8">
          ...
          >
            ...
            <PsychologyMoodTracker trades={filteredTrades} strategies={strategies} />
            <RiskRewardChart trades={filteredTrades} strategies={strategies} />
            <Card>
              <CardHeader>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    ...
                    <p className="text-lg font-bold">{activeFilteredTrades.length}</p>
                    <p className="text-xs text-gray-500">Active strategies only</p>
                  </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {strategyPerformance.map((item, index) => (
                    <tr key={index} className="border-b hover:bg-50">
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{activeFilteredTrades.length}</span>
              </div>
              <div className="flex justify-between">
